...
ncbi_ensembl( Self, DnDir, Opts ) :-
     ...
     csv_ids_map( SpeciesF, 'GeneID', ensg, Mtx, GEnsGF, [header(row('NCBI ID','Ensembl Gene'))|Lens] ),
     csv_ids_map( SpeciesF, ensg, 'GeneID', Mtx, EnsGGF, [header(row('Ensembl Gene','NCBI ID'))|Rens] ),
     Lenp = [prefix(ncbi),to_value_1(pos_integer),to_value_2(pfx_by_de_v('ENS')),datetime(DnDt),source(Url)],
     append( Lenp, Opts, ALenp ),
     csv_ids_map( SpeciesF, ncbi, ensp, Mtx, GEnsPF, [header(row('NCBI ID','Ensembl Protein'))|ALenp] ),
