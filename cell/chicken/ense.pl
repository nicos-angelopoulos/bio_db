:- module( bio_db_gallus_ense, [
                ...
                map_ense_gg6a_enst_chrl/5,
                map_ense_gg6a_ensg_chrl/5,
                map_ense_gg6a_enst_ensg/2,
                map_ense_gg6a_ensg_symb/2
             ]
         ).
...
/**  map_ense_gg6a_ensg_symb( ?EnsG, ?MgimID ).

Ensembl gene id to symbol with data drawn from Ensembl.

==
?- map_ense_gg6a_ensg_symb( A, B ).
==
*/
map_ense_gg6a_ensg_symb( EnsG, Symb ) :-
    bio_db:bio_db_serve( map_ense_gg6a_ensg_symb(EnsG,Symb) ).

/**  map_ense_gg6a_enst_chrl( +EnsT, -Chr, -Start, -End, -Dir ).
...
*/
map_ense_gg6a_enst_chrl( EnsT, Chr, Start, End, Dir ) :-
    bio_db:bio_db_serve( map_ense_gg6a_enst_chrl(EnsT,Chr,Start,End,Dir) ).


/*  map_ense_gg6a_ensg_chrl( +EnsG, -Chr, -Start, -End, -Dir ).
...
*/
map_ense_gg6a_ensg_chrl( EnsG, Chr, Start, End, Dir ) :-
    bio_db:bio_db_serve( map_ense_gg6a_ensg_chrl(EnsG,Chr,Start,End,Dir) ).

/**  map_ense_gg6a_enst_ensg( +EnsT, -EnsG ).
...
*/
map_ense_gg6a_enst_ensg( EnsT, EnsG ) :-
    bio_db:bio_db_serve( map_ense_gg6a_enst_ensg(EnsT,EnsG) ).
